...
public:
    ...
    TEST_METHOD(TestRAIntegrationGetMenu)
    {
        ...
        Assert::AreEqual(14U, pMenu->num_items);
        int nIndex = 0;
        AssertMenuItem(pMenu, nIndex++, IDM_RA_HARDCORE_MODE, "&Hardcore Mode");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_NON_HARDCORE_WARNING, "Non-Hardcore &Warning");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_ONLY_HARDCORE_UNLOCKS, "Only Hardcore Unloc&ks");
        AssertMenuSeparator(pMenu, nIndex++);
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_OPENALL, "&Open All");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_ACHIEVEMENTS, "Assets Li&st");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_ACHIEVEMENTEDITOR, "Assets &Editor");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_MEMORYFINDER, "&Memory Inspector");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_MEMORYBOOKMARKS, "Memory &Bookmarks");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_CODENOTES, "Memory &Notes");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_PARSERICHPRESENCE, "Rich &Presence Monitor");
        AssertMenuSeparator(pMenu, nIndex++);
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_POINTERFINDER, "Pointer &Finder");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_POINTERINSPECTOR, "Pointer &Inspector");

        runtime.mockLoginService.Login("User", "ApiToken");

        pMenu = _Rcheevos_RAIntegrationGetMenu();
        Assert::AreEqual(20U, pMenu->num_items);
        nIndex = 0;
        AssertMenuItem(pMenu, nIndex++, IDM_RA_OPENUSERPAGE, "Open my &User Page");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_OPENGAMEPAGE, "Open this &Game's Page");
        AssertMenuSeparator(pMenu, nIndex++);
        AssertMenuItem(pMenu, nIndex++, IDM_RA_HARDCORE_MODE, "&Hardcore Mode");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_NON_HARDCORE_WARNING, "Non-Hardcore &Warning");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_ONLY_HARDCORE_UNLOCKS, "Only Hardcore Unloc&ks");
        ...
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_POINTERINSPECTOR, "Pointer &Inspector");
        AssertMenuSeparator(pMenu, nIndex++);
        AssertMenuItem(pMenu, nIndex++, IDM_RA_REPORTBROKENACHIEVEMENTS, "&Report Achievement Problem");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_GETROMCHECKSUM, "View Game H&ash");
        ...
        AssertMenuItem(pMenu, nIndex++, IDM_RA_OPENGAMEPAGE, "Open this &Game's Page");
        AssertMenuSeparator(pMenu, nIndex++);
        AssertMenuItemChecked(pMenu, nIndex++, IDM_RA_HARDCORE_MODE, "&Hardcore Mode");
        AssertMenuItemChecked(pMenu, nIndex++, IDM_RA_NON_HARDCORE_WARNING, "Non-Hardcore &Warning");
        AssertMenuItemChecked(pMenu, nIndex++, IDM_RA_ONLY_HARDCORE_UNLOCKS, "Only Hardcore Unloc&ks");
        AssertMenuSeparator(pMenu, nIndex++);
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_OPENALL, "&Open All");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_ACHIEVEMENTS, "Assets Li&st");
        AssertMenuItem(pMenu, nIndex++, IDM_RA_FILES_ACHIEVEMENTEDITOR, "Assets &Editor");
